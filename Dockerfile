...
RUN wget http://www.nagare.org/download/nagare-0.5.1.tar.gz && tar xvf nagare-0.5.1.tar.gz && rm -rf nagare-0.5.1.tar.gz
...
RUN python virtualenv-15.1.0/virtualenv.py /work/nagare-0.5.1 && /work/nagare-0.5.1/bin/easy_install 'nagare[full]' && \
		echo "export PATH=$PATH:/work/nagare-0.5.1/bin" >> /etc/profile
RUN pip install kansha
